...
export async function fetchProducts(db: Db) {
  const [productRows, imageRows, linkRows, categoryRows, variantRows] = await Promise.all([
  ...
  return productRows.map((row) =>
    toProduct(row, imagesByProduct.get(row.id) ?? [], slugsByProduct.get(row.id) ?? [], variantsByProduct.get(row.id) ?? []),
  );
}
...
export async function fetchProductBySlug(db: Db, slug: string) {
  ...
  const [images, links, categoryRows, variants] = await Promise.all([
    ...
    db.select().from(categories),
    db.select().from(productVariants).where(and(eq(productVariants.productId, row.id), eq(productVariants.active, true))),
  ]);
  const slugByCategoryId = new Map(categoryRows.map((category) => [category.id, category.slug]));
  const slugs: string[] = [];
  ...
  return toProduct(row, images, slugs, variants);
}
