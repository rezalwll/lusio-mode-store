...
function StorefrontCategoryLink({ category, className, children }: { category: StorefrontCategory; className: string; children: ReactNode }) {
  if (category.categorySlug) return <Link href={`/category/${category.categorySlug}`} className={className}>{children}</Link>;
    ...
    <>
          ...
          <div className="relative min-h-[540px] overflow-hidden bg-ink lg:min-h-[660px]">
            ...
            <div className="relative z-10 flex h-full min-h-[540px] max-w-2xl flex-col justify-end p-6 text-white sm:p-10 lg:min-h-[660px] lg:p-14">
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                <Link href="/shop?q=&category=&sort=newest" className="inline-flex h-12 items-center gap-3 bg-white px-6 text-xs font-black text-ink transition hover:bg-[#eee9e2]">{settings.heroPrimaryCta || "خرید کالکشن"} <ArrowLeft className="size-4" /></Link>
                <Link href="/shop?q=&category=&sort=popular" className="inline-flex h-12 items-center gap-3 border border-white/50 px-6 text-xs font-bold text-white transition hover:bg-white hover:text-ink">پرفروش‌های هفته</Link>
              </div>
            </div>
          </div>
          ...
          <div className="grid min-h-[420px] grid-rows-[.78fr_1.22fr] gap-px bg-white/70">
            <div className="flex flex-col justify-between bg-sand p-7 sm:p-9">
              ...
              <div>
                ...
                <Link href="/category/men-shirt" className="mt-5 inline-flex items-center gap-2 border-b border-ink pb-1 text-[11px] font-black">دیدن ادیت روزمره <ArrowUpLeft className="size-4" /></Link>
              </div>
            </div>
            {spotlight && (
              <Link href={`/product/${spotlight.slug}`} className="group relative min-h-[300px] overflow-hidden bg-stone-200">
      ...
      {settings.showNewArrivals !== false && <section className="container-site py-16 sm:py-24" style={{ order: orderOf("new") }}>
        <div className="mb-8 flex items-end justify-between gap-4">
          ...
          <Link href="/shop?q=&category=&sort=newest" className="flex shrink-0 items-center gap-2 text-xs font-bold hover:text-brand">همه تازه‌ها <ArrowLeft className="size-4" /></Link>
        </div>
      ...
      {settings.showSaleProducts !== false && saleProducts.length > 0 && <section className="bg-[#f7e9e5]" style={{ order: orderOf("sale") }}>
        <div className="container-site py-16 sm:py-24">
          <div className="mb-8 flex items-end justify-between gap-4">
            ...
            <Link href="/shop?q=&category=&sort=sale" className="flex shrink-0 items-center gap-2 text-xs font-bold hover:text-brand">همه تخفیف‌ها <ArrowLeft className="size-4" /></Link>
          </div>
        ...
        <div className="grid min-h-[520px] overflow-hidden border border-black/5 bg-[#f1e7df] lg:grid-cols-[.8fr_1.2fr]">
          <div className="flex flex-col justify-between bg-mint p-7 sm:p-10 lg:p-14">
            ...
            <div className="py-12">
              ...
              <Link href={shopHref("", "", saleProducts.length ? "sale" : "popular")} className="mt-7 inline-flex h-11 items-center gap-3 bg-ink px-5 text-xs font-black text-white transition hover:bg-brand">{settings.festivalCta || "ورود به جشنواره"} <ArrowLeft className="size-4" /></Link>
            </div>
          ...
          <div className="relative min-h-[420px] bg-clay">
            ...
            <Link href={`/product/${festivalProduct.slug}`} className="absolute bottom-5 left-5 right-5 flex items-center justify-between bg-[#fffdfb] p-4 text-ink sm:bottom-7 sm:left-7 sm:right-auto sm:w-[330px]">
          ...
          <div className="container-site grid min-h-[560px] lg:grid-cols-[.8fr_1.2fr]">
            <div className="flex flex-col justify-between py-10 lg:py-14 lg:pl-14">
              ...
              <div className="py-12 lg:py-8">
                ...
                <Link href={`/product/${editorialProduct.slug}`} className="mt-7 inline-flex h-11 items-center gap-3 border border-white/35 px-5 text-xs font-bold transition hover:bg-white hover:text-ink">{settings.editorialCta || "دیدن این انتخاب"} <ArrowLeft className="size-4" /></Link>
              </div>
      ...
      {settings.showBestSellers !== false && <section className="container-site py-16 sm:py-24" style={{ order: orderOf("best") }}>
        <div className="mb-8 flex items-end justify-between gap-4">
          ...
          <Link href="/shop?q=&category=&sort=popular" className="flex shrink-0 items-center gap-2 text-xs font-bold hover:text-brand">همه پرفروش‌ها <ArrowLeft className="size-4" /></Link>
        </div>
