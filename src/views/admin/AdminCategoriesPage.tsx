...
import { archiveCategoryAction, saveCategoryAction } from "@/server/actions/catalog";
import type { Category, Product } from "@/types/store";

const empty = { name: "", slug: "", description: "", image: "", active: true };

export function AdminCategoriesPage({ categories, products }: { categories: Category[]; products: Product[] }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); if (!form.name.trim() || !form.slug.trim()) { toast.error("نام و نامک دسته‌بندی الزامی است"); return; } const result = await saveCategoryAction({ id: editing?.id, parent: editing?.parent ?? 0, ...form, slug: form.slug.trim().toLowerCase().replace(/[^a-z0-9\u0600-\u06ff]+/gi, "-").replace(/^-|-$/g, "") }); if (!result.ok) return toast.error(result.message); setOpen(false); toast.success("دسته‌بندی ذخیره شد"); router.refresh(); }
  async function remove(category: Category) { const count = products.filter((item) => item.category === category.slug || item.categorySlugs.includes(category.slug)).length; if (count > 0) { toast.error(`این دسته‌بندی ${toFa(count)} محصول دارد و قابل بایگانی نیست`); return; } if (window.confirm(`دسته‌بندی «${category.name}» بایگانی شود؟`)) { const result = await archiveCategoryAction(category.id); if (!result.ok) return toast.error(result.message); toast.success("دسته‌بندی بایگانی شد"); router.refresh(); } }
  return (
    <div className="mx-auto max-w-[1500px] space-y-6"><div className="flex items-end justify-between gap-4"><div><p className="text-[10px] font-bold text-brand">ساختار کاتالوگ</p><h1 className="mt-1 text-2xl font-black">دسته‌بندی‌ها</h1><p className="mt-1.5 text-[10px] text-muted">دسته‌های قابل نمایش در فروشگاه را سازمان‌دهی کنید.</p></div><Button onClick={() => start()}><Plus className="size-4" />دسته جدید</Button></div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? "ویرایش دسته‌بندی" : "دسته‌بندی جدید"}><form onSubmit={submit}><div className="grid gap-4 p-5 sm:p-6"><Field label="نام دسته‌بندی"><Input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></Field><Field label="نامک انگلیسی" hint="برای آدرس صفحه استفاده می‌شود"><Input value={form.slug} disabled={Boolean(editing)} onChange={(event) => setForm({ ...form, slug: event.target.value })} dir="ltr" /></Field><BannerImageField label="تصویر دسته‌بندی" value={form.image} onChange={(image) => setForm({ ...form, image })} mobile /><Field label="توضیحات"><Textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} rows={4} /></Field><label className="flex items-center justify-between rounded-xl border border-border p-3 text-[10px] font-bold">نمایش در فروشگاه<input type="checkbox" checked={form.active} onChange={(event) => setForm({ ...form, active: event.target.checked })} className="size-4 accent-ink" /></label></div><div className="flex justify-end gap-2 border-t border-border p-4"><Button type="button" variant="outline" onClick={() => setOpen(false)}>انصراف</Button><Button type="submit">ذخیره</Button></div></form></Modal>
    </div>
  );
}
