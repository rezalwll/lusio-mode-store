...
export function AdminSettingsPage({ settings }: { settings: StoreSettings }) {
  ...
  async function submit(values: SettingsForm) {
    try {
      await saveStoreSettingsAction({ ...settings, ...values, shippingCost: Number(values.shippingCost), freeShippingThreshold: Number(values.freeShippingThreshold), monthlySalesTarget: Number(values.monthlySalesTarget), lowStockThreshold: Number(values.lowStockThreshold) });
      router.refresh();
      toast.success("تنظیمات فروشگاه ذخیره شد");
    } catch { toast.error("ذخیره تنظیمات انجام نشد"); }
  }
