...
export function AdminOrdersPage({ orders }: { orders: Order[] }) {
  ...
  async function saveDetails() {
    if (!selected) return;
    const result = await updateOrderAction({ id: selected.id, trackingCode: selected.trackingCode?.trim(), internalNote: selected.internalNote?.trim() });
    if (!result.ok) return toast.error(result.message);
    ...
    <div className="mx-auto max-w-[1500px] space-y-6">
      ...
      <Modal open={Boolean(selected)} onClose={() => setSelected(null)} title={`جزئیات سفارش ${selected?.id || ""}`} description={selected ? formatDate(selected.createdAt) : ""} className="max-w-2xl">
        {selected && <div className="space-y-6 p-5 sm:p-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2"><label><span className="mb-2 block text-[10px] font-bold">وضعیت سفارش</span><select value={selected.status} onChange={(event) => changeStatus(selected.id, event.target.value as OrderStatus)} className="h-11 w-full rounded-xl border border-border px-3 text-xs outline-none">{statuses.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><div className="rounded-xl border border-border px-3 py-2"><span className="block text-[10px] font-bold">وضعیت پرداخت (فقط خواندنی)</span><strong className="mt-1 block text-xs">{paymentStatusLabels[selected.paymentStatus]}</strong></div></div>
          {selected.paymentAttempt && <dl className="grid gap-2 rounded-xl border border-border bg-stone-50 p-4 text-[10px] sm:grid-cols-2"><div><dt className="text-muted">درگاه / وضعیت تلاش</dt><dd className="mt-1 font-bold" dir="ltr">{selected.paymentAttempt.provider} / {selected.paymentAttempt.status}</dd></div><div><dt className="text-muted">شناسه مرجع</dt><dd className="mt-1 break-all font-bold" dir="ltr">{selected.paymentAttempt.transactionId || selected.paymentAttempt.authority || "—"}</dd></div>{selected.paymentAttempt.verifiedAt && <div><dt className="text-muted">زمان تأیید</dt><dd className="mt-1">{formatDate(selected.paymentAttempt.verifiedAt)}</dd></div>}{selected.paymentAttempt.failureMessage && <div className="sm:col-span-2"><dt className="text-muted">خطای درگاه</dt><dd className="mt-1 text-red-700">{selected.paymentAttempt.failureMessage}</dd></div>}</dl>}
