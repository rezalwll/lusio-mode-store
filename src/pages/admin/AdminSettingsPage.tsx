...
import { Download, Image, RotateCcw, Save, Settings2, Truck, Upload } from "lucide-react";
...
export function AdminSettingsPage() {
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<SettingsForm>({
    ...
    defaultValues: { ...settings, shippingCost: String(settings.shippingCost), freeShippingThreshold: String(settings.freeShippingThreshold) },
  });
  const heroImage = watch("heroImage");

  function submit(values: SettingsForm) { updateSettings({ ...values, shippingCost: Number(values.shippingCost), freeShippingThreshold: Number(values.freeShippingThreshold) }); toast.success("تنظیمات فروشگاه ذخیره شد"); }
  function exportData() {
  ...
  return (
    <div className="mx-auto max-w-[1200px] space-y-6"><div><p className="text-[10px] font-bold text-brand">کنترل کامل فروشگاه</p><h1 className="mt-1 text-2xl font-black">تنظیمات و محتوا</h1><p className="mt-1.5 text-[10px] text-muted">محتوای صفحه اصلی، اطلاعات تماس، ارسال و داده‌های فروشگاه را مدیریت کنید.</p></div>
      <form onSubmit={handleSubmit(submit)} className="space-y-5">
        ...
        <section className="rounded-2xl border border-black/5 bg-white p-5 sm:p-6"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-rose-50 text-brand"><Image className="size-5" /></span><div><h2 className="text-xs font-black">محتوای صفحه اصلی</h2><p className="mt-1 text-[9px] text-muted">تیتر و تصاویر بنر اصلی فروشگاه</p></div></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><Field label="عنوان اصلی" error={errors.heroTitle?.message} className="sm:col-span-2"><Input {...register("heroTitle")} /></Field><Field label="توضیح زیر عنوان" error={errors.heroSubtitle?.message} className="sm:col-span-2"><Textarea {...register("heroSubtitle")} rows={3} /></Field><Field label="تصویر دسکتاپ" error={errors.heroImage?.message}><Input {...register("heroImage")} dir="ltr" /></Field><Field label="تصویر موبایل" error={errors.heroMobileImage?.message}><Input {...register("heroMobileImage")} dir="ltr" /></Field>{heroImage && <div className="sm:col-span-2"><p className="mb-2 text-[9px] font-bold">پیش‌نمایش تصویر اصلی</p><img src={heroImage} alt="پیش‌نمایش بنر" className="max-h-64 w-full rounded-xl object-cover" /></div>}</div></section>
