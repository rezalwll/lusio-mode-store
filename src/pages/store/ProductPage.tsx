    ...
    <div className="container-site py-7 sm:py-10">
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[1.05fr_.95fr] lg:gap-14">
        ...
        <section className="self-center lg:py-8">
          ...
          {product.colors.length > 0 && <div className="mt-7"><div className="flex items-center justify-between"><label className="text-xs font-black">رنگ</label><span className="text-[10px] text-muted">{color}</span></div><div className="mt-3 flex flex-wrap gap-2">{product.colors.map((item) => <button key={item} type="button" onClick={() => setColor(item)} className={cn("rounded-xl border px-3 py-2 text-[11px] font-bold", color === item ? "border-ink bg-ink text-white" : "border-border hover:border-ink")}>{item}</button>)}</div></div>}
          {product.sizes.length > 0 && <div className="mt-6"><div className="flex items-center justify-between"><label className="text-xs font-black">سایز</label><button type="button" className="text-[10px] text-muted underline">راهنمای سایز</button></div><div className="mt-3 flex flex-wrap gap-2" dir="ltr">{product.sizes.map((item) => <button key={item} type="button" onClick={() => setSize(item)} className={cn("min-w-11 rounded-xl border px-3 py-2 text-[11px] font-bold", size === item ? "border-ink bg-ink text-white" : "border-border hover:border-ink")}>{item}</button>)}</div></div>}

          <div className="mt-8 flex gap-2">
            <div className="flex h-13 items-center rounded-xl border border-border"><button type="button" className="grid size-10 place-items-center" onClick={() => setQuantity(Math.max(1, quantity - 1))}><Minus className="size-4" /></button><span className="min-w-8 text-center text-xs font-bold">{toFa(quantity)}</span><button type="button" className="grid size-10 place-items-center" onClick={() => setQuantity(Math.min(product.stock, quantity + 1))}><Plus className="size-4" /></button></div>
            <Button size="lg" className="flex-1" onClick={add} disabled={product.stock === 0}>{product.stock === 0 ? "ناموجود" : "افزودن به سبد خرید"}</Button>
            <Button size="icon" variant="outline" className="size-13" aria-label="علاقه‌مندی"><Heart className="size-5" /></Button>
          </div>
          <p className={`mt-3 text-[10px] font-bold ${product.stock > 0 ? "text-emerald-700" : "text-rose-700"}`}>{product.stock > 0 ? `${toFa(product.stock)} عدد در انبار موجود است` : "این محصول فعلاً موجود نیست"}</p>
